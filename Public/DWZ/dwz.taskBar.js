...
(function($){
	...
	$.taskBar = {
		...
		addDialog: function(id, title){
			...
			if (!task[0]) {
				var taskFrag = '<li id="#taskid#"><div class="taskbutton"><span>#title#</span></div><div class="close"><i class="icon-remove-sign"></i></div></li>';
		...
		restoreDialog:function(obj){
			...
			$(".resizable").css({top: "-7px",left:$(task).position().left,height:$(task).outerHeight(),width:$(task).outerWidth()
		...
		show:function(){
			if (this._taskBar.is(":hidden")) {
				...
				$('#menu_c_s').css("margin-top","15px");
				
				//alert(this._taskBar.outerHeight());
				this._taskBar.css("top", "-41px").show();
		 ...
		 */
		hide:function(){
			$('#menu_c_s').css("margin-top","10px");
			//alert(this._taskBar.outerHeight(true));
			this._taskBar.animate({
				top: "-41px"
			}, 500,function(){
